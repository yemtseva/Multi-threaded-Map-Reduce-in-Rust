...
fn main() {
    ...
    let mut thread_results : Vec<thread::JoinHandle<_>> = Vec::new();

    for i in 0..2 {
        let arg = xs[i].clone();
        thread_results.push(thread::spawn(move || map_data(&arg)));
    }

    for _ in 0..2 {
        let thread = thread_results.remove(0);
        let sums = thread.join().unwrap();
        intermediate_sums.push(sums);
    ...
    let mut thread_results2 : Vec<thread::JoinHandle<_>> = Vec::new();

    for i in 0..num_partitions {
        let arg = partitions[i].clone();
        thread_results2.push(thread::spawn(move || map_data(&arg)));
    }

    for _ in 0..num_partitions {
        let thread = thread_results2.remove(0);
        let sums = thread.join().unwrap();
        threaded_sums.push(sums);
    ...
    let threaded_sum = reduce_data(&threaded_sums);
    
    // 6. Prints the final sum computed by reduce_data
    println!("Sum = {}", threaded_sum);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let range = v.len() / num_partitions;
    let mut index = 0;
    let mut end;
